...
Use `test:`, `ci:`, `docs:`, or `chore(release):` for repository tests, automation, repository documentation, or release maintenance that does not change installed skill content. When one commit changes a skill and its tests, select the type from the skill's catalog effect.
