...
- **怎么用：** “使用 design-verifiable-playtest，把这个 `gda-balancing` 假设及其维护来源转化为一个聚焦的可玩实验。”
...
- **怎么用：** “使用 entropy-review 找出这份计划中不成比例的复杂度，并给出更小、更容易撤回的替代方案。”

#### [`handle-review`](../skills/handle-review/SKILL.md)

- **做什么：** 在采纳 review 意见前，先对照当前变更、需求、约束和运行证据验证这些意见。
- **为什么：** Review 可能指出了真实问题，却高估其影响，或给出不合适的解决机制。
- **何时使用：** 适合处理 PR review，或判断一条意见应该原样采纳、调整后采纳，还是拒绝。
...
- **怎么用：** “使用 reconcile 找出受这次决策变更影响的全部资料，为每项事实恢复唯一权威来源，并更新由它派生的引用。”
...
- **为什么：** 下一次工作应该从已知状态继续，而不是重新发现项目进展，或丢失尚未处理的工作。
- **何时使用：** 适合在一次工作结束时，或把任务交给另一个 agent 之前使用。
- **怎么用：** “使用 state 记录今天的结果、建议的下一项任务，以及必须延续到下次工作的未完成事项。”

#### [`pitfalls`](../skills/pitfalls/SKILL.md)

- **做什么：** 通过项目级 `PITFALLS.md`，避免重复遇到已经验证过的环境、工具、权限、sandbox 和调用问题。
- **为什么：** 已确认的操作经验很容易丢失，后续工作会再次执行同样的失败命令，或重复错误的环境假设。
- **何时使用：** 在工具密集型工作开始前、已知坑点可能适用时，或刚确认一个可复用的操作失败后使用。
- **怎么用：** “运行项目工具前先使用 pitfalls；只有确认新的环境经验后，再把它记录下来。”
