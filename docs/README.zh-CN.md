...
- **做什么：** 沿需求、假设、决策和补丁回溯停滞的任务，从第一性原理重新检查支持范围与设计边界。
- **为什么：** 小补丁可能保留错误前提，让总体复杂度不断增加，却未推动原始任务通过验收。
- **何时使用：** 功能或修复反复不收敛，或辅助机制持续增加却没有推动任务接近验收时使用；不能仅凭迭代次数判定设计错误。
- **怎么用：** “使用 backtrace-review 调查这项任务反复返工的原因，验证关键假设，并给出复杂度适当的恢复路径。”
...
这些 Skills 会使用部分相同的证据，但各自回答不同的主要问题。应选择能够处理当前问题的最小审查范围；需要深入判断时，把工作交给相应 Skill，不要重复另一项 Skill 的完整流程。`skill-review` 仍然专门用于审查 Skill 包。
...
当一项需求、决策、范围或术语的变更得到批准后，使用 `reconcile` 修复已经过时的依赖工件。如果只有一个主要问题，也可以独立使用其中一项 Skill。
